...
# BASE_PATH = "/u/scr/xlisali/contrast_LM/transformers/examples/text-generation/"
BASE_PATH = "./"
...
def main():
    ...
    if args.task_mode == 'data2text':
        ...
        if QUICK_CHECK:
            ...
            decode_mode = 'beam'
            curr_dir = os.path.join(BASE_PATH,
                                    args.gen_dir,
                                    '{}_{}_{}'.format(temp, split_file, decode_mode))
            print(curr_dir)
            gold_dir = os.path.join(BASE_PATH,
                                    args.gen_dir,
            ...
            out_handle1 = open("HELLOHELLOHELLO", 'w')
    ...
    for prompt_idx, prompt_text in enumerate(prompt_text_lst):
        ...
        if args.tuning_mode == 'prefixtune' or args.tuning_mode == 'bothtune':
            ...
            if decode_mode == 'nucleus':
                ...
                # print("WRONG: BEAM DECODE")
                print("DECODE NUCLEUS fake")
                output_sequences = gpt2.generate(
                    input_ids=input_ids,
                    emb_match=None,
                    control_code=None,
                ...
                #############################
                output_sequences1 = gpt2.generate(
                    input_ids=input_ids,
                    emb_match=None,
                    control_code=None,
                    past_key_values=prompt,
            ...
            # print(decode_mode)
            if decode_mode == 'nucleus':
                print("DECODE MODE NUCLEUS fineetune")
                ...
                print("WRONG: BEAM DECODE finetune")
                output_sequences = model.generate(
                    input_ids=input_ids,
                    emb_match=None,
                    control_code=control_code,
        ...
        if len(output_sequences1.shape) > 2:
            output_sequences1.squeeze_()
        ...
        generated_sequences1 = []

        if QUICK_CHECK:
            for generated_sequence_idx, generated_sequence in enumerate(output_sequences1):
                # print("=== GENERATED SEQUENCE {} ===".format(generated_sequence_idx + 1))
                # args.stop_token = tokenizer.eos_token
                generated_sequence = generated_sequence.tolist()
                ...
                generated_sequences1.append(total_sequence)
                # print(total_sequence)
        else:
            for generated_sequence_idx, generated_sequence in enumerate(output_sequences1):
                # print("=== GENERATED SEQUENCE {} ===".format(generated_sequence_idx + 1))
                ...
                if text_output:
                    print(text_output, file=out_handle1)
                else:
                    print('Error', file=out_handle1)
        # print()
    ...
    if args.task_mode == 'data2text':
        ...
        print("THIS IS THE SCORE WE NEED TO PAY ATTENTION TO")
        print(out_file_eval, '\n', gold_dir, '\n', curr_dir)

        os.system("python /content/drive/MyDrive/E2E-prefixtune/e2e-metrics/measure_scores.py "
        ...
        os.system("bash dart-eval/run_eval_on_webnlg.sh "
...
if __name__ == "__main__":
    ...
    # BASICALLYY what i did was created this file HELLOHELLOHELLO which you should compare to the output of out_file_eval and if theyre the same then for some reason beam and nuucleus have same output
