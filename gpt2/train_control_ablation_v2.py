...
class PrefixTuning(GPT2PreTrainedModel):
    ...
    def get_prompt_p5_prefix_infix(self, src, control_code=None, gpt2=None, bsz=None, attn_mask=None):
        # VERSION1. infixing by taking in the last layer of the hidden states as input.
        ...
        input_tokens = self.input_tokens.unsqueeze(0).expand(bsz, -1).to(self.device)
        temp_control = self.wte(input_tokens)
        past_key_values_prefix = self.control_trans(temp_control) #bsz, seqlen, layer*emb
        bsz, seqlen, _ = past_key_values_prefix.shape
        past_key_values_prefix = past_key_values_prefix.view(bsz, seqlen, self.match_n_layer * 2, self.match_n_head,
                                               self.match_n_embd)
        past_key_values_prefix = self.dropout(past_key_values_prefix)
        past_key_values_prefix = past_key_values_prefix.permute([2, 0, 3, 1, 4])
        
        
        temp_control = self.wte(input_tokens)
        past_key_values_infix = self.control_trans(temp_control)  # bsz, seqlen, layer*emb
        bsz, seqlen, _ = past_key_values_infix.shape
        past_key_values_infix = past_key_values_infix.view(bsz, seqlen, self.match_n_layer * 2, self.match_n_head,
                                               self.match_n_embd)
        past_key_values_infix = self.dropout(past_key_values_infix)
        past_key_values_infix = past_key_values_infix.permute([2, 0, 3, 1, 4])


        temp_emb = self.wte2(input_tokens)
        src_emb = gpt2.transformer.wte(src)
        total_emb = torch.cat([temp_emb, src_emb, temp_emb], dim=1) #bsz, seqlen, dim
        # print("Total_emb shape: ", total_emb.shape)
        src_out = gpt2(inputs_embeds=total_emb, attention_mask=attn_mask ,use_cache=True, return_dict=True)
        src_past_key_vals = src_out.past_key_values
        src_past_key_vals = torch.cat(src_past_key_vals, dim=0)
        # print(src_past_key_vals.shape, past_key_values.shape) # the src should be longer than past.
        # get a zero mask.
        _, src_len = src.shape
        nl, nb, nh, _, ndim = past_key_values_infix.shape
        zero_mask = torch.zeros(nl, nb, nh, src_len, ndim).to(self.device)
        # print(zero_mask.shape, past_key_values.shape)
        past_key_values = torch.cat([past_key_values_prefix, zero_mask, past_key_values_infix], dim=3)
        #print("past_key_values shape: ", past_key_values.shape)
        #print("hello:", torch.cat([zero_mask, past_key_values_infix], dim=3).shape)
        past_key_values = past_key_values + src_past_key_vals
    ...
    def forward_infix(self,
        ...
        **kwargs,
        ...
        if self.mode_para == 2:
            past_key_values_prompt = self.get_prompt_p5_prefix_infix(src, gpt2=gpt2_model, bsz=bsz)
            attention_mask = torch.cat([src_attn, src_attn, src_attn, tgt_attn], dim=1) # bsz, seqlen
        else:
            #print("we got here")
            infix_attn = torch.ones(bsz, self.preseqlen).bool().to(self.device)
            prefix_attn = torch.ones(bsz, self.preseqlen).bool().to(self.device)
            attention_mask = torch.cat([prefix_attn, src_attn, infix_attn, tgt_attn], dim=1)  # bsz, seqlen
            partial_attn_mask = torch.cat([prefix_attn, src_attn, infix_attn], dim=1)  # bsz, seqlen
            past_key_values_prompt = self.get_prompt_p5_prefix_infix(src, None, gpt2=gpt2_model, bsz=bsz, attn_mask=partial_attn_mask)
